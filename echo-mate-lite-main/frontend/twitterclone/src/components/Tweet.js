...
import { TWEET_API_END_POINT, getBaseURL } from '../utils/constant';
...
const Tweet = ({ tweet }) => {
    ...
    const getImageURL = (imagePath) => {
        if (!imagePath) {
            return "https://pbs.twimg.com/profile_images/1703261403237502976/W0SFbJVS_400x400.jpg";
        }
        ...
        if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
            ...
            if (imagePath.includes('localhost:8080')) {
                ...
                return imagePath.replace('http://localhost:8080', baseURL);
            }
            return imagePath;
        }
        ...
        <div className='border-b border-gray-200'>
            <div>
                <div className='flex p-4'>
                    <Avatar 
                        src={getImageURL(tweet?.userDetails[0]?.profilePicture)} 
                        size="40" 
                        round={true} 
                    />
